...
```text
==================================================================
 caruca_v2 dev template: Cleanup
...
### 🗄️ Database Schema Cleanup with Drizzle ORM

**For projects using Drizzle ORM:**

When removing unused database tables, **NEVER** use `npm run db:generate:custom` for schema changes. Instead, use Drizzle's schema-first approach:

```bash
# ✅ CORRECT: Delete schema files and let Drizzle generate migration
# 1. Delete unused schema files (e.g., conversations.ts, messages.ts)
rm lib/drizzle/schema/conversations.ts lib/drizzle/schema/messages.ts

# 2. Update schema/index.ts to remove exports
# Remove: export * from "./conversations";

# 3. Generate migration from schema changes
npm run db:generate

# 4. MANDATORY: Create down migration for rollback
# Follow ai_docs/dev_templates/drizzle_down_migration.md

# 5. Apply migration
npm run db:migrate

# ❌ WRONG: Don't use custom migrations for schema changes
# npm run db:generate:custom  # Only for RLS, triggers, functions
```

**Why this matters:**
- Drizzle tracks schema changes automatically through TypeScript files
- Custom migrations should only be used for SQL not expressible in Drizzle (RLS, triggers, functions)
- Deleting schema files and running `db:generate` creates proper DROP TABLE statements
- This maintains Drizzle's migration chain integrity
