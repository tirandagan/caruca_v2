...
const users = await db
  ...
  .from(usersTable)
  .where(
    sql`${usersTable.age} > 18 AND ${usersTable.isActive} = true AND ${usersTable.role} IN ('admin', 'moderator')`,
  );
...
const result = await db
  ...
  .from(posts)
  .where(
    sql<string>`to_tsvector('simple', ${posts.content}) @@ to_tsquery('search term')`,
  );
...
const query = await db.select().from(users).where(eq(users.name, userInput)); // Safely parameterized
